...
namespace CitySkylines0._5alphabeta
{
    ...
    public class CarManager
    {
        ...
        private void SetNextEdge(Car car, Edge nextEdge)
        {
            ...
            var intersection = car.currentEdge.intersections.FirstOrDefault(node => node.connectedEdges.Contains(nextEdge));

            if (intersection != null)
            {
                car.currentTrack[0] = intersection.coords;
                car.currentTrack[1] = (nextEdge.a == intersection.coords) ? nextEdge.b : nextEdge.a;
            }
            else
            {
        ...
        public void MoveCarOnEdge(Car car)
        {
            ...
            foreach (Car carCheck in cars)
            {
                PointF carCheckCoords = carCheck.currentPosition;
                if (carCheck != car && carCoords.X < carCheckCoords.X + 10 && carCoords.X > carCheckCoords.X - 10 && carCoords.Y < carCheckCoords.Y + 10 && carCoords.Y > carCheckCoords.Y - 10) { return; }
                else
                {
        ...
        public void CreateCarRoute(Car car)
        {
            ...
            while (unvisited.Count > 0)
            {
                ...
                if (current == car.destinationEdge) { break; }
